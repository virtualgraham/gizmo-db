...
use gizmo_db::graph::iterator::recursive::{Recursive};
use gizmo_db::graph::iterator::{Shape, Morphism};
use gizmo_db::graph::refs::{pre_fetched, Namer};
use gizmo_db::graph::value::{Value};
use gizmo_db::graph::linksto::{LinksTo};
use gizmo_db::graph::hasa::{HasA};
...
fn test_recursive_next() {
    ...
    while r.borrow_mut().next() {
        let re = r.borrow().result().unwrap();
        got.push(re.unwrap_value().to_string());
    }
...
fn test_recursive_next_path() {
    ...
    while r.borrow_mut().next() {
        ...
        let re = res[&"person".to_string()].unwrap_value().to_string();
        got.push(re);
