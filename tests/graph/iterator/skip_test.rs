...
use gizmo_db::graph::iterator::skip::Skip;
...
fn test_not_iterator_basics() {
    ...
    let expect_sz = all_it.borrow_mut().stats().unwrap();
    let sz = u.borrow_mut().stats().unwrap();
    ...
    for v in &[1,2,3] {
        assert!(!uc.borrow_mut().contains(&Ref::new_i64_node(*v as i64)))
    }
    for v in &[4,5] {
        assert!(uc.borrow_mut().contains(&Ref::new_i64_node(*v as i64)))
    }
