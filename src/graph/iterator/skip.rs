...
impl Shape for Skip {
    ...
    fn stats(&mut self) -> Result<Costs, String> {
        let mut primary_stats = self.primary_it.borrow_mut().stats()?;
    ...
    fn optimize(&mut self) -> Option<Rc<RefCell<dyn Shape>>> {
        let optimized_primary_it = self.primary_it.borrow_mut().optimize();
        if self.skip == 0 {
...
impl Base for SkipNext {
    ...
    fn next_path(&mut self) -> bool {
        while self.skipped < self.skip {
            if !self.primary_it.borrow_mut().next_path() {
                return false
            }
            self.skipped += 1;
        }
        return self.primary_it.borrow_mut().next_path()
    }
...
impl Scanner for SkipNext {
    fn next(&mut self) -> bool {
        while self.skipped < self.skip {
            if !self.primary_it.borrow_mut().next() {
                return false
            }
            self.skipped += 1;
        }
...
impl Index for SkipContains {
    fn contains(&mut self, val:&refs::Ref) -> bool {
        ...
        while self.skipped <= self.skip {
            ...
            in_next_path = false;
            if !self.primary_it.borrow_mut().contains(val) {
                return false
            }
            self.skipped += 1;
        ...
        if in_next_path && self.primary_it.borrow_mut().next_path() {
            return true
        }

        return self.primary_it.borrow_mut().contains(val)
    }
}